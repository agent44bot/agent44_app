# Owner-only checkable plans shown at /admin/plan and summarized in the
# Monday PlanStatusReportJob email. Plain Ruby: the plan text lives here, and
# completion lives in kv_settings as "<key>:done:<step_id>" = completion time
# so checking off survives deploys and records when each step happened.
#
# ALL is newest-first; the first entry is the default view. Each plan's :key
# prefixes its kv settings (the June plan keeps its original "june_plan" key so
# existing check-offs survive). Step ids must be stable.
class OwnerPlan
  Step = Struct.new(:id, :title, :detail, :links, :section, :done_at, keyword_init: true) do
    def done? = !done_at.nil?
  end
  ...
  ALL = [ GROWTH_2027, JUNE_2026 ].freeze

  attr_reader :slug, :key, :title, :tab, :subtitle, :intro, :footer, :sections

  def self.all = ALL.map { |h| new(h) }
  def self.default = new(ALL.first)

  def self.find(slug)
    h = ALL.find { |p| p[:slug] == slug.to_s }
    h && new(h)
  end

  def initialize(h)
    @slug, @key, @title, @tab, @subtitle, @intro, @footer, @sections =
      h.values_at(:slug, :key, :title, :tab, :subtitle, :intro, :footer, :sections)
  end

  # Flat steps with their section title and completion time.
  def steps
    @steps ||= sections.flat_map do |s|
      s[:steps].map do |st|
        Step.new(id: st[:id], title: st[:title], detail: st[:detail], links: st[:links],
                 section: s[:title], done_at: Setting.time(done_key(st[:id])))
      end
    end
  end

  def step?(id) = sections.any? { |s| s[:steps].any? { |st| st[:id] == id } }
  def done_key(id) = "#{key}:done:#{id}"

  def total = steps.size
  def completed = steps.count(&:done?)
  def percent = total.positive? ? (100.0 * completed / total).round : 0
  def complete? = total.positive? && completed == total

  def done_at(id) = steps.find { |s| s.id == id }&.done_at
  ...
  # Steps checked off inside [from, to).
  def done_between(from, to) = steps.select { |s| s.done_at && s.done_at >= from && s.done_at < to }
  def remaining = steps.reject(&:done?)
end
